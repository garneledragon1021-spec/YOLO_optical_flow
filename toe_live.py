#!/usr/bin/env python3
#toe_live.py
"""リアルタイムに足元のキーポイントを追跡するプログラム。

Ultralytics YOLO Pose は実行時に全身を推定するが、負荷を抑えるため N フレームごと、
...
TOE_LANDMARKS = {
    # COCO pose keypoints: left/right ankle.
    # 標準YOLO Poseモデルにはつま先キーポイントがないため、足首を追跡する。
    "left": 15,
    "right": 16,
}
...
def detect_toe(
    ...
    """YOLO Poseから指定側の足首キーポイントをピクセル座標で取得する。"""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Realtime YOLO Pose ankle tracker")
    ...
    parser.add_argument("--device", default=None, help="YOLO device, e.g. cpu, 0 (default: auto)")
    parser.add_argument("--model", default="yolo11n-pose.pt", help="Ultralytics YOLO pose model path/name")
